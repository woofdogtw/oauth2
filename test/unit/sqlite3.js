...
describe('unit.sqlite3', function() {
  before(initFunc);
  ...
  describe('models', function() {
    ...
    describe('models.oauth2', function() {
      ...
      it('getAuthorizationCode() with wrong code',
    ...
    describe('models.user', function() {
      ...
      it('addUser() with default info', models.user.addUserDefault);
    ...
    describe('models.client', function() {
      ...
      it('getClient() with wrong client ID', models.client.getClientWrong);
  ...
  describe('routes', function() {
    if (config.oauth2.db.engine !== 'sqlite3') {
      return;
    }
...
function initFunc(done) {
  ...
  async.waterfall([
    // Connect database.
    function(cb) {
      model = require('../../models/oauth2-sqlite3');
      const dbConn = require('../../models/oauth2-sqlite3/connection');
      dbConn.on('connect', function() {
        db = dbConn.connection;
        cb(null);
      });
    },
    // Wait for DB tables ready.
    function(cb) {
      (function check() {
        checkDbReady(db, function(err) {
          if (err) {
...
function exitFunc(done) {
  const conn = require('../../models/oauth2-sqlite3/connection');
  ...
  conn.close(function() {
    ...
    fs.unlink(conn._path, done);
  });
}

function checkDbReady(db, callback) {
  async.waterfall([
    // Check user table.
    function(cb) {
      db.run('SELECT COUNT("userId") FROM user', function(err, row) {
        cb(err);
      });
    },
    // Check client table.
    function(cb) {
      db.run('SELECT COUNT("id") FROM client', function(err, row) {
        cb(err);
      });
    },
    // Check token table.
    function(cb) {
      db.run('SELECT COUNT("accessToken") FROM token', function(err, row) {
        cb(err);
      });
    },
    // Check authCode table.
    function(cb) {
      db.run('SELECT COUNT("code") FROM authCode', function(err, row) {
        cb(err);
      });
    }
  ], callback);
...
function dataInitFunc(done) {
  const db = require('../../models/oauth2-sqlite3/connection').connection;
...
function dataRemoveFunc(done) {
  ...
  async.waterfall([
    // Remove user.
    function(cb) {
      db.run('DELETE FROM user', cb);
    },
    // Remove client.
    function(cb) {
      db.run('DELETE FROM client', cb);
    },
    // Remove token.
    function(cb) {
      db.run('DELETE FROM token', cb);
    },
    // Remove authorization code.
    function(cb) {
      db.run('DELETE FROM authCode', cb);
    }
  ], function(err) {
    done(err);
  });
...
function initUser(db, callback) {
  const sql = 'INSERT INTO user ' +
    '(userId,email,created,validated,expired,disabled,roles,password,salt,' +
    'name,info) VALUES ($userId,$email,$created,$validated,$expired,' +
    '$disabled,$roles,$password,$salt,$name,$info)';
  ...
  (function insert() {
    ...
    const params = {
      ...
      $validated: (user.validated ? user.validated.getTime() : null),
      $expired: (user.expired ? user.expired.getTime() : null),
...
function initClient(db, callback) {
  const sql = 'INSERT INTO client ' +
    '(id,created,clientSecret,redirectUris,scopes,grants,userId,name,image)' +
    ' VALUES ($id,$created,$clientSecret,$redirectUris,$scopes,$grants,' +
    '$userId,$name,$image)';
  ...
  (function insert() {
    ...
    const client = testParams.client[clients[index]];
    const params = {
      $id: client.id,
      $created: client.created.getTime(),
...
function initToken(db, callback) {
  const sql = 'INSERT INTO token ' +
    '(accessToken,accessTokenExpiresAt,refreshToken,refreshTokenExpiresAt,' +
    'scope,client,user) VALUES ($accessToken,$accessTokenExpiresAt,' +
    '$refreshToken,$refreshTokenExpiresAt,$scope,$client,$user)';
  ...
  (function insert() {
    ...
    const token = testParams.token[tokens[index]];
    const params = {
      ...
      $scope: token.scope,
      $client: JSON.stringify(token.client),
      $user: JSON.stringify(token.user)
    };
    db.run(sql, params, (err) => {
      if (err) {
        return callback(err);
...
function initAuthCode(db, callback) {
  const sql = 'INSERT INTO authCode ' +
    '(code,expiresAt,redirectUri,scope,client,user) VALUES ' +
    '($code,$expiresAt,$redirectUri,$scope,$client,$user)';
  const params = {
    $code: testParams.authCode.udca.code,
    $expiresAt: testParams.authCode.udca.expiresAt.getTime(),
    $redirectUri: testParams.authCode.udca.redirectUri,
    $scope: testParams.authCode.udca.scope,
    $client: JSON.stringify(testParams.authCode.udca.client),
    $user: JSON.stringify(testParams.authCode.udca.user)
  };
